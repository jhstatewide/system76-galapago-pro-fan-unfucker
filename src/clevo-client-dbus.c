/*
 ============================================================================
 Name        : clevo-client-dbus.c
 Author      : System76 Fan Control DBus Client
 Version     : 1.0
 Description : Modern DBus client for Clevo fan control daemon

 This client provides a command-line interface to interact with the clevo-daemon
 using DBus for efficient local communication with intelligent broadcasting.
...
static int call_method_string(const char* method_name, const char* arg);
static int get_status_response(char* response, size_t size);
...
static int live_stats_display(void);
...
int main(int argc, char* argv[]) {
    ...
    // Connect to DBus
    if (!connect_to_dbus()) {
        fprintf(stderr, "Failed to connect to DBus. Is clevo-daemon running?\n");
        return EXIT_FAILURE;
    }
    ...
    switch (config.type) {
        case CMD_STATUS:
            {
                ...
                if (get_status_response(response, sizeof(response)) == 0) {
        ...
        case CMD_SET_FAN:
            {
                if (call_method("SetFanDuty", config.fan_duty) == 0) {
        ...
        case CMD_SET_AUTO:
            {
                if (call_method_bool("SetAutoMode", TRUE) == 0) {
                    printf("Auto mode enabled\n");
                } else {
                    fprintf(stderr, "Failed to enable auto mode\n");
                    return EXIT_FAILURE;
                }
            }
            break;
        ...
        case CMD_SET_TARGET_TEMP:
            {
                // Note: This would need to be implemented in the daemon
                printf("Target temperature setting not yet implemented in DBus interface\n");
            }
            break;
        ...
        case CMD_SET_MAX_DUTY_CHANGE:
            {
                if (call_method("SetMaxDutyChange", config.max_duty_change_rate) == 0) {
                    printf("Max duty change rate set to %d%%\n", config.max_duty_change_rate);
                } else {
                    fprintf(stderr, "Failed to set max duty change rate\n");
                    return EXIT_FAILURE;
                }
            }
            break;
        ...
        case CMD_GET_MAX_DUTY_CHANGE:
            {
                ...
                printf("Get max duty change not yet implemented in DBus interface\n");
            }
            break;
            
        case CMD_SET_MAX_DUTY_INCREASE:
            {
                printf("Set max duty increase not yet implemented in DBus interface\n");
            }
            break;
            
        case CMD_GET_MAX_DUTY_INCREASE:
            {
                printf("Get max duty increase not yet implemented in DBus interface\n");
            }
            break;
            
        case CMD_SET_MAX_DUTY_DECREASE:
            {
                printf("Set max duty decrease not yet implemented in DBus interface\n");
            }
            break;
            
        case CMD_GET_MAX_DUTY_DECREASE:
            {
                printf("Get max duty decrease not yet implemented in DBus interface\n");
            }
            break;
            
        case CMD_RECOVER_TEMP:
            {
                printf("Temperature recovery not yet implemented in DBus interface\n");
            }
            break;
        ...
        case CMD_LIVE_STATS:
            {
                ...
                live_stats_init();
                while (running) {
                    int display_status = live_stats_display();
...
static int connect_to_dbus(void) {
    ...
    // Connect to system bus (daemon runs on system bus)
...
static int get_status_response(char* response, size_t size) {
    ...
    if (dbus_message_iter_get_arg_type(&iter) == DBUS_TYPE_STRING) {
        dbus_message_iter_get_basic(&iter, &response_str);
        strncpy(response, response_str, size - 1);
        response[size - 1] = '\0';
    }
...
static void print_help(void) {
    printf("Usage: clevo-client-dbus [OPTIONS] COMMAND\n\n");
    printf("Commands:\n");
    ...
    printf("Examples:\n");
    printf("  clevo-client-dbus --status\n");
    printf("  clevo-client-dbus --monitor 5\n");
    printf("  clevo-client-dbus --live-stats 0.1\n");
    printf("  clevo-client-dbus --set-fan 80\n");
    printf("  clevo-client-dbus --set-max-duty-change 10\n");
    printf("  clevo-client-dbus --get-max-duty-change\n");
    printf("  clevo-client-dbus --json --status\n");
}
...
// Live stats implementation (simplified for DBus)
static void live_stats_init(void) {
